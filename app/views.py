...
from config import (
    PATH_UDPIPE_MODEL, chkr, BASE_DIR, numeric_features, UWL, CONNECTORS,
    important_features, feature_mapping, feature_feedback_to_improve, feature_feedback_best, feature_hints)
import pickle
...
def predict_multi(text, feature_dict):
    num_values = [feature_dict.get(feature_value, 0) for feature_value in important_features]
    X = np.array(num_values).reshape(1, -1)
...
def recommendation(feature_dict):
    ...
    for obj in MEAN:
        ...
        if feature_name in features_to_improve:
            table = table.replace('<td style="width: 100px;text-align: center;">' + str(feature_name),
                                  '<td class="feature_rec_name" style="width: 300px;color: darkred;text-align: center;'
            ...
            table = table.replace('<td style="width: 100px;'
                                  'text-align: center;">' + str(feature_name),
                                  '<td class="feature_rec_name" style="width: 300px;color: darkgreen;text-align: center;'
                                  'font-weight: bolder;">' + str(feature_mapping.get(feature_name)))
...
def hint_features(feature_name):
    return get_feature_hint(feature_name)


def get_feature_hint(feature_name):
    hint_text = feature_hints.get(feature_name)
    if not hint_text:
        hint_text = ''
    res = '<p class="text_recommendation"><span class="feature_hint_name">' + feature_name + '</span>' + hint_text + '</p>'
    return res
